 ...
 * 
 * The S-Box is a substitution table used in the Rijndael algorithm for byte substitution during encryption and decryption.
 * It is a 256-byte table that maps each possible byte value to another byte value.
 * This lookup table is used in the Rijndael algorithm to perform the SubBytes transformation.
 * 
 ...
 * 
 * This lookup table is used in the Rijndael algorithm for SubBytes transformation.
 * It maps each input byte to a corresponding output byte using a predefined substitution pattern.
 * The table is a 256-byte array where each element represents the output byte for a specific input byte.
 * The values in the table are in hexadecimal format.
 * 
 * @note This table is used for encryption and decryption operations in the Rijndael algorithm.
 * 
 ...
 * 
 * @note The Rijndael algorithm uses these round constants during the key expansion process.
 * 
...
/**
 * Retrieves the value from the S-box lookup table based on the given input number.
 *
 * @param num The input number for which the S-box value needs to be retrieved.
 * @return The value from the S-box lookup table corresponding to the input number.
 */
 ...
 * 3. XORs the output of the r_con operation with the first part (leftmost) of the word.
 *
...
void aes_key_schedule_core(unsigned char *word, int iteration) {
    ...
    // XOR the output of the r_con operation with i to the first part (leftmost) only
